...
output_JAGS <- function(jags.1, mix, source, output_options=list(
  ...
  if(!output_options[[9]]){  # if 'suppress XY plot' is NOT checked
    ...
    if(output_options[[10]]){ # svalue(plot_xy_save_pdf)
      mypath <- file.path(paste(getwd(),"/",output_options[[11]],"_Lead(Pb)_p.pdf",sep=""))  # svalue(plot_xy_name)
      dev.copy2pdf(file=mypath)
    }
    if(output_options[[20]]){ # svalue(plot_xy_save_png)
      mypath <- file.path(paste(getwd(),"/",output_options[[11]],"_Lead(Pb)_p.png",sep=""))  # svalue(plot_xy_name)
      dev.copy(png,mypath)
    }
  ...
  if(!output_options[[3]]){   # if 'suppress posterior plots' is NOT checked
    ...
    if(mix$n.fe == 0){ # only if there are no fixed effects, otherwise p.global is meaningless
      ...
      print(ggplot2::ggplot(df, ggplot2::aes(x=x, fill=sources, colour=sources)) +
              ...
              ggplot2::theme_bw() +
              ggplot2::xlab("Proportion of Lead (Pb)") +
      ...
      if(output_options[[4]]){ # svalue(plot_post_save_pdf)
        mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_global.pdf",sep=""))  # svalue(plot_post_name)
        dev.copy2pdf(file=mypath)
      }
      if(output_options[[18]]){ # svalue(plot_post_save_png)
        mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_global.png",sep=""))  # svalue(plot_post_name)
        dev.copy(png,mypath)
      }
    }
    ...
    if(n.effects >= 1 & mix$n.fe != 2){
      ...
      for(f1 in 1:mix$FAC[[1]]$levels){    # formerly factor1_levels
        ...
        # Save the plot to file
        if(output_options[[4]]){ # svalue(plot_post_save_pdf)
          mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_",mix$FAC[[1]]$labels[f1],".pdf",sep=""))  # svalue(plot_post_name), factor1_names
          dev.copy2pdf(file=mypath)
        }
        if(output_options[[18]]){ # svalue(plot_post_save_png)
          mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_",mix$FAC[[1]]$labels[f1],".png",sep=""))  # svalue(plot_post_name), factor1_names
          dev.copy(png,mypath)
        }
      ...
      if(n.re==2){
        ...
        for(f2 in 1:mix$FAC[[2]]$levels){  # formerly factor2_levels
          ...
          if(output_options[[4]]){ # svalue(plot_post_save_pdf)
            mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_",mix$FAC[[2]]$labels[f2],".pdf",sep="")) #  svalue(plot_post_name), factor2_names
            dev.copy2pdf(file=mypath)
          }
          if(output_options[[18]]){  # svalue(plot_post_save_png)
            mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_",mix$FAC[[2]]$labels[f2],".png",sep="")) #  svalue(plot_post_name), factor2_names
            dev.copy(png,mypath)
          }
    ...
    if(mix$fere){
      for(f1 in 1:mix$FAC[[1]]$levels) {
        for(f2 in fac2_lookup[[f1]]){
          ...
          # Save the plot as a pdf file
          if(output_options[[4]]){ # svalue(plot_post_save_pdf)
            mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_",mix$FAC[[1]]$labels[f1],"_",mix$FAC[[2]]$labels[f2],".pdf",sep="")) #  svalue(plot_post_name), factor2_names
            dev.copy2pdf(file=mypath)
          }
          if(output_options[[18]]){  # svalue(plot_post_save_png)
            mypath <- file.path(paste(getwd(),"/",output_options[[5]],"_Lead(Pb)_p_",mix$FAC[[1]]$labels[f1],"_",mix$FAC[[2]]$labels[f2],".png",sep="")) #  svalue(plot_post_name), factor2_names
            dev.copy(png,mypath)
          }
        } # f2
      } # f1
